                ...
                <div className="rounded-lg overflow-auto max-h-[400px] shadow">
                    {
                        recentSearches.map((recentSearch, idx) => {
                            return (
                                <div onClick={() => handleSearch(recentSearch.id)} key={idx} className="flex items-center border-b last:border-b-0 mx-auto px-4 py-2 shadow bg-white cursor-pointer hover:bg-gray-100 transition">
                                    {recentSearch.avatar?.url ? (
                                        <img className='relative mr-4 flex-shrink-0 object-cover object-center rounded-full w-10 h-10' src={`${recentSearch.avatar.url}?size=64`} alt={recentSearch.username} />
                                    ) : (
                                        <div className='relative mr-4 flex-shrink-0 flex items-center justify-center font-bold rounded-full w-10 h-10 bg-gray-100'>
                                            <svg className="fill-current text-gray-600" width="20" height="20" viewBox="0 0 28 20">
                                    ...
                                    <div className='font-semibold flex-grow'>
                                        {recentSearch.username ? recentSearch.username : (
                                            <span className='font-medium'>{recentSearch.id}</span>
                                        )}
                                        {(recentSearch.discriminator && recentSearch.discriminator !== "0") && (
                                            <small className='text-sm text-gray-500'>
                                                #{recentSearch.discriminator}
                                            </small>
                                        )}
                                    </div>
                                    <button className='ml-auto focus:outline-none w-10 h-10 rounded-full hover:bg-red-100 transition' aria-label='Remove Item Button' onClick={(e) => handleRemoveFromHistory(e, idx)}>
